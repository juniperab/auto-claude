...
module AutoClaude
  class EnvironmentDiagnosticTest < Minitest::Test
    def test_environment_variables_passed_to_claude
      ...
      Open3.stub :popen3, lambda { |*args|
        ...
        Dir.mktmpdir do |tmpdir|
          # Save original PWD
          original_pwd = ENV.fetch("PWD", nil)
          ...
          if captured_env.nil?
            ...
            puts "This means PWD=#{ENV.fetch("PWD", nil)} is passed to Claude"
          end
        end
      end
    end
    ...
    def test_check_claude_md_locations
      ...
      # Check project root
      claude_md_locations << File.join(project_root, "CLAUDE.md") if File.exist?(File.join(project_root, "CLAUDE.md"))
      ...
      3.times do
        ...
        claude_md_locations << claude_md_path if File.exist?(claude_md_path)
      end
      ...
      claude_md_locations << home_claude if File.exist?(home_claude)
    ...
    def test_subprocess_pwd_behavior
      ...
      Dir.mktmpdir do |tmpdir|
        ...
        output = nil
        Open3.popen3("pwd", chdir: tmpdir) do |_stdin, stdout, _stderr, _wait_thread|
        ...
        # Test environment
        env_output = nil
        Open3.popen3("sh", "-c", "echo PWD=$PWD", chdir: tmpdir) do |_stdin, stdout, _stderr, _wait_thread|
