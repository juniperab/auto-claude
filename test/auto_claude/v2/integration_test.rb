...
class AutoClaude::V2::IntegrationTest < Minitest::Test
  def test_full_flow_with_memory_output
    output = AutoClaude::V2::Output::Memory.new
    client = AutoClaude::V2::Client.new(output: output)
  ...
  def test_with_file_logging
    Tempfile.create("v2_test_log") do |tmpfile|
      file_output = AutoClaude::V2::Output::File.new(tmpfile.path)
      terminal_output = AutoClaude::V2::Output::Memory.new
      multiplexer = AutoClaude::V2::Output::Multiplexer.new([terminal_output, file_output])
      
      client = AutoClaude::V2::Client.new(output: multiplexer)
  ...
  def test_callback_system
    ...
    Open3.stub :popen3, create_mock_popen(mock_response) do
      ...
      assert_equal ["TextMessage", "ToolUseMessage", "ToolResultMessage", "ResultMessage"], messages_seen
    end
  end

  def test_v2_module_convenience_method
    mock_response = <<~JSON
      {"type": "result", "subtype": "success", "result": "Quick result"}
    JSON
    
    Open3.stub :popen3, create_mock_popen(mock_response) do
      result = AutoClaude::V2.run("Quick test", output: AutoClaude::V2::Output::Memory.new)
  ...
  def create_mock_popen(stdout_content, exit_code = 0, stderr_content = "")
    -> (script_path, &block) {
      ...
      assert File.exist?(script_path) if script_path.is_a?(String)
